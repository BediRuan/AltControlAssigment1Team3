...
namespace FronkonGames.Retro.VintageFilters
{
  ...
  public sealed partial class VintageFilters
  {
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static VintageFilters Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is VintageFilters)
              return scriptableRendererData.rendererFeatures[i] as VintageFilters;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static VintageFilters[] Instances
    {
      get
      {
        if (UniversalRenderPipeline.asset != null)
        {
          ...
          List<VintageFilters> effects = new();
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is VintageFilters)
                  effects.Add(feature as VintageFilters);
          }
